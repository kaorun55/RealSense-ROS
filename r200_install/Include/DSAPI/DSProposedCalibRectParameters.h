...
#include <stdint.h>
/*
  This file contains the proposed new DSCalibRectParameters format, adapted to systems with a "platform" aka "fourth" camera
*/
...
const int DS_MAX_NUM_INTRINSICS_THIRD = 3;
const int DS_MAX_NUM_INTRINSICS_PLATFORM = 3;      ///< Max number native resolutions the third camera can have (e.g. 1920x1080 and 640x480)
...
const int DS_MAX_NUM_RECTIFIED_MODES_PLATFORM = 4; 
...
struct DSCalibRectParameters
{
    ...
    uint16_t versionNumber; ///< Version of this format

    uint8_t numIntrinsicsRight;     ///< Number of right cameras < DS_MAX_NUM_INTRINSICS_RIGHT
    uint8_t numIntrinsicsThird;     ///< Number of native resolutions of third camera < DS_MAX_NUM_INTRINSICS_THIRD
    uint8_t numRectifiedModesLR;    ///< Number of rectified LR resolution modes < DS_MAX_NUM_RECTIFIED_MODES_LR
    uint8_t numRectifiedModesThird; ///< Number of rectified Third resolution modes < DS_MAX_NUM_RECTIFIED_MODES_THIRD
